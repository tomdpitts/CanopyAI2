...
CFG_TILE = TargetConfig(grid=256, stride=8)     # 2048 tile at 8px
...
class TileData:
    def __init__(self, arm):
        ...
        for t, v in self.gt.items():
            ...
            e = encode(bx, CFG_TILE)
            self.enc[t] = {k: torch.from_numpy(e[k]) for k in
                           ("heatmap", "offset", "size", "reg_mask")}
            # canopy ignore at the 256 grid (downsample the 2048 canopy mask)
            self.ign[t] = torch.from_numpy(
                canopy_cell_mask(canopy_px(v["canopy"]), CFG_TILE.grid))
            self.boxes[t] = bx
...
def train(args):
    ...
    print(f"[{args.tag}] device={device} in_dim={in_dim} params={npar:.2f}M "
          f"full-tile 256-grid train/val={len(tr)}/{len(va)} seed={args.seed}",
          flush=True)
