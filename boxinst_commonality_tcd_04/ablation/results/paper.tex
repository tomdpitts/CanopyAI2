...
In short, the development of RS Computer Vision techniques in sparse-canopy areas for the purposes of Tree Canopy monitoring has been underexplored, and yet represents a significant biome in measurement of global carbon sequestration. Recent annotated datasets such as SelvaBox (Baudchon et al., 2026) \& SelvaMask (Duguay et al., 2026) have broadened geographic coverage, and Restor Foundation's OAM-TCD dataset (Veitch-Michaelis et al., 2024) in particular incorporates a variety of biomes across the globe, including urban, tundra, and savannah.
...
\caption{Instance segmentation on the OAM-TCD 439-image holdout. Every row was produced and scored by us under one frozen protocol (using standard COCOeval). All are finetuned on OAM-TCD training set, \emph{Imgs} is the number of OAM-TCD training images seen. Restor and SelvaBox were not finetuned by us, the others were.}
\label{tab:oamtcd439}
...
Both versions use batch ($16$) and learning rate ($10^{-4}$), so backbone is the only difference between the two. The architecture emits at most $100$ instances per $1024^2$ subtile, which cannot be raised without discarding the COCO initialisation. Box2Mask emits at most 100 instances per $1024^2$ subtile (which overlap 50\% in a 3x3 grid to cover $2048^2$) and reaches that ceiling on $28.8\%$ of test subtiles, although only $1.2\%$ actually hold more than 100 ground-truth crowns. Removing every tile containing such a subtile moves this row by $+0.008$ AP$_{50}$.
\end{tablenotes}
\end{threeparttable}
\end{table}


Tables~\ref{tab:oamtcd439} and~\ref{tab:neon-detection} summarize our model's performance across mAP (COCO mean Average Precision) for OAM-TCD and instance-level detection metrics at IoU 0.4 for NEONTreeEvaluation, following the authors' conventions. We report evaluations alongside baseline model results for comparison. For simplicity, consistency, and replicability, we use seed values starting at 0 and incremented by 1 (i.e. 0,1,2 for 3 seed). Every baseline in Table~\ref{tab:oamtcd439} was re-run and re-scored by us under a single protocol. Each baseline was run in its own native prediction geometry, so only the measurement is held fixed, and we comment in the table notes on any configuration choice that materially affects a row. 

Our scorer is standard pycocotools COCOeval (iouType segm then bbox), pooled over images and with a single category 'tree'. Full details are in Appendix~\ref{app:protocol}. We include all 439 images in the GT always, without dropping images containing zero predictions.

Table~\ref{tab:oamtcd439} includes Box2Mask, the only baseline drawn from LACE's own supervision class of 900 boxes. With its strongest backbone it reaches $0.540$ mask AP$_{50}$ against LACE's $0.663$ on identical training crops, an identical tiling grid and the same scorer. It finds crowns well but delineates them fairly coarsely. It should be noted that Box2Mask was developed on COCO objects spanning hundreds of pixels, whereas the median OAM-TCD crown at $0.1$~m/px is significantly smaller.
...
To test model efficacy on savannah and Dryland biomes, we take a 'sparse-canopy' slice of the OAM-TCD data: $236$ tiles carrying $14{,}937$ crowns, none of which appears among the $900$ training images. No model is retrained for it -- the saved per-seed predictions are simply re-scored.

\begin{table}[H]
\caption{Instance segmentation on the sparse-canopy OAM-TCD holdout ($236$ tiles, $14{,}937$ crowns, no tile shared with the $900$ training images). Canopy is scored as \texttt{iscrowd} throughout, as in Table~\ref{tab:oamtcd439}. $\Delta$ is LACE minus DetecTree2. LACE bands are $\mathrm{ddof}=1$ over detector seeds $0$--$2$; DetecTree2 is deterministic at a single seed.\label{tab:sparse236}} \newcolumntype{C}{>{\centering\arraybackslash}X}
...
\midrule
DetecTree2 (fine-tuned, full masks) & $0.6118$ & $0.2155$ & $0.2800$ & $0.5897$ \\
\textbf{LACE (ours, boxes only)} & $\mathbf{0.6913}$ & $\mathbf{0.2326}$ & $\mathbf{0.3148}$
  & $\mathbf{0.6442}$ \\
 & {\footnotesize$\pm$0.0095} & {\footnotesize$\pm$0.0107} & {\footnotesize$\pm$0.0069}
  & {\footnotesize$\pm$0.0272} \\
\midrule
$\Delta$ & $+0.080$ & $+0.017$ & $+0.035$ & $+0.055$ \\
\bottomrule
\end{tabularx}
\noindent{\footnotesize{Scored with the same frozen COCOeval protocol as Table~\ref{tab:oamtcd439} (pycocotools, masks at $512^2$, maxDets $600$, canopy as \texttt{iscrowd}), against the sparse ground truth rather than the $439$. LACE leads on every metric and on every seed. Both methods score higher here than on the full $439$ (LACE $0.691$ against $0.663$; DetecTree2 $0.612$ against $0.597$), so the open-canopy regime is easier for both and the margin is not an artefact of one model preferring it; the margin is nonetheless larger here ($+0.080$) than on the $439$ ($+0.066$). The contrast is sharper at strict IoU: on the $439$ DetecTree2 \emph{beats} LACE at AP$_{75}$ ($0.175$ against $0.165$), whereas here LACE still leads it ($0.233$ against $0.216$). Sixteen of the $236$ tiles are drawn from the $439$ itself; they were never trained on, and the three inference scalars were selected on the $108$ validation images rather than on the $439$ (Section~\ref{sec:inference}).}}
\end{table}




\paragraph{Box-to-Mask Module Against a Promptable Segmenter}

Tables~\ref{tab:oamtcd439} and~\ref{tab:sparse236} measure whole systems, in which detection and mask generation are entangled: a method with better boxes hands its mask stage an easier problem, so a difference in mask AP is not by itself a statement about the mask module. To isolate the module we remove detection altogether and prompt each candidate with the \emph{ground-truth} crown boxes. Because prompt box $i$ is the tight box of ground-truth crown $i$, the mask returned is scored against that crown and no other: there is no matching step, no ranking, no score threshold, and no detector. Table~\ref{tab:gtbox} reports the outcome over all $439$ test tiles and $25{,}692$ crowns, at the native $2048^2$ raster and, as a sensitivity, at the $512^2$ raster used elsewhere in this paper. Our module leads on every measure at both resolutions. Because each module is evaluated on the same crowns, the comparison is paired, so we report the fraction of individual crowns on which each competitor produces the better mask rather than relying on a difference of means: at $2048$, SAM~3 wins on $40.1\%$ of crowns and its fine-tuned variant on $19.9\%$.

Three features of the table deserve comment. First, the scoring raster matters, and it matters asymmetrically. Moving from $512$ to $2048$ costs our module $0.016$ mean IoU against SAM~3's $0.009$, because a coarse raster smooths away boundary error that a $8$~px-lattice masker cannot avoid making. The effect is concentrated at strict thresholds: our advantage at IoU~$\ge 0.9$ falls from $2.4\times$ to $1.4\times$. On a $512$ raster a one-pixel boundary error leaves a median IoU of only $0.71$ and just $1.3\%$ of crowns above $0.9$, against $0.90$ and $51.4\%$ at $2048$, so an IoU~$\ge 0.9$ threshold scored at $512$ largely measures whether a mask is pixel-exact rather than whether it is accurate. We therefore report $2048$ as the headline and treat the $512$ figures as the comparison to the rest of the paper. Second, fine-tuning \emph{harms} SAM~3 here ($0.623$ against $0.741$), which we attribute to resolution rather than domain: the SelvaMask checkpoint was fitted on UAV imagery at $1.3$--$3.5$~cm/px and applied to OAM-TCD at $10$~cm/px, and it is run at the $1777$~px tile size its authors deploy, so the arm is not starved. Third, the one measure on which our module is clearly worse is double assignment, and the gap is large: at $2048$ it claims overlapping pixels at roughly twenty times the rate present in the ground truth itself, where both SAM arms sit within a small multiple of that floor. A per-box generative posterior has no mechanism forbidding two neighbouring crowns from claiming the same pixel, and touching crowns are $39\%$ of this dataset; the accuracy gain and this failure mode are two consequences of the same locality assumption. We note finally that this experiment scopes a narrower claim than Table~\ref{tab:oamtcd439}: it establishes that our module is the stronger box-to-mask converter among those that accept a box prompt, not that its masks are the most accurate obtainable by any means.

\begin{table}[H]
\begin{threeparttable}
\caption{Box-to-mask modules prompted with \emph{ground-truth} crown boxes, OAM-TCD $439$-tile holdout, $25{,}692$ crowns. Every module receives the identical prompt set, so the comparison is paired crown-for-crown and detection quality plays no part. The upper block scores masks at the imagery's native $2048^2$; the lower repeats the experiment at the $512^2$ raster used elsewhere in this paper.\label{tab:gtbox}} \newcolumntype{C}{>{\centering\arraybackslash}X}
\setlength{\tabcolsep}{4pt}
...
\shortstack{\textbf{Double}\\\textbf{assign.}}\tnote{a} &
\shortstack{\textbf{Crowns}\\\textbf{won}}\tnote{b} \\
\midrule
\multicolumn{7}{l}{\emph{Scored at $2048^2$ (native imagery resolution)}} \\
\textbf{LACE commonality EM (ours)} & $\mathbf{0.770}$ & $\mathbf{0.979}$ & $\mathbf{0.659}$
  & $\mathbf{0.051}$ & $0.030$ & --- \\
SAM 3, zero-shot\tnote{c}            & $0.741$ & $0.956$ & $0.545$ & $0.037$
  & $0.008$ & $40.1\%$ \\
SAM 3, SelvaMask fine-tuned\tnote{d} & $0.623$ & $0.789$ & $0.274$ & $0.011$
  & $\mathbf{0.002}$ & $19.9\%$ \\
\midrule
\multicolumn{7}{l}{\emph{Scored at $512^2$ (the raster used elsewhere in this paper)}} \\
\textbf{LACE commonality EM (ours)} & $\mathbf{0.786}$ & $\mathbf{0.986}$ & $\mathbf{0.710}$
  & $\mathbf{0.089}$ & $0.036$ & --- \\
SAM 3, zero-shot                     & $0.749$ & $0.978$ & $0.565$ & $0.037$
  & $0.008$ & $36.3\%$ \\
SAM 3, SelvaMask fine-tuned          & $0.605$ & $0.769$ & $0.234$ & $0.008$
  & $\mathbf{0.002}$ & $14.5\%$ \\
\bottomrule
\end{tabularx}
\begin{tablenotes}[flushleft]\footnotesize
\item[a] $(\sum_i m_i - |\bigcup_i m_i|)/|\bigcup_i m_i|$ per tile, averaged over tiles. Lower is
better. The ground-truth crowns themselves overlap at $0.0015$ at $2048$ and $0.0048$ at $512$,
which is the floor against which the column should be read, not zero.
\item[b] Fraction of the $25{,}692$ crowns on which that module returns a higher IoU than ours.
\item[c] \texttt{facebook/sam3} image model, box-prompted at the native $2048^2$ tile.
\item[d] \texttt{CanopyRS/sam3-multi-selvabox-selvamask-FT}, run at the $1777$~px tile size its
wrapper defaults to, the configuration its authors deploy; its masks are therefore interpolated
rather than native at the $2048$ scoring raster. All $685$ checkpoint tensors were asserted to
match before inference.
\end{tablenotes}
\end{threeparttable}
\end{table}

The oracle-box column of Table~\ref{tab:components} measures the same quantity at $512$ on the $108$ validation images ($0.789$ for the deployed configuration) and agrees with the $0.786$ obtained here on the $439$-tile holdout at that raster, so the module's behaviour on ideal boxes is stable across splits.
...
\end{table}
Both factors earn their place: dropping either loses roughly a third of the gain, on validation as well as test, and the two are near-additive alone ($+0.0234$ and $+0.0222$ against $+0.0365$ together), so they carry weakly overlapping information rather than one riding on the other.
...
The clearest evidence that Equation~\eqref{eq:rerank} is not tuned to the test distribution is the sparse split. The functional form was selected on $108$ validation images drawn from the \emph{training} distribution; on $236$ tiles sharing no image with training, the gain is larger rather than smaller ($+0.0354$ against $+0.0325$). Because no weight is fitted, there is no vehicle by which distribution-specific information could have been carried across.
...
DetecTree2, fine-tuned (1 seed)   & full masks  & $\mathbf{0.7928}$   & $\mathbf{0.9212}$ \\
\bottomrule
\end{tabularx}
\noindent{\footnotesize{Ours and the filled-box baseline are evaluated on identical matched pairs and are exactly paired; DetecTree2 matches its own detections ($20{,}897$ true positives against approximately $20{,}743$) and that comparison is therefore approximate, though the two populations are close in size. Tolerances above $1$~px saturate ($\ge 0.90$ at $2$~px, $\ge 0.99$ at $5$~px) and are reported for completeness only.}}
\end{table}

Box supervision costs little in boundary placement but not nothing: we exceed a filled box by $0.104$ at the tightest measurable tolerance, while sitting $0.032$ below a mask-supervised model. The cost is clearer at strict IoU. Our lead over DetecTree2 is $+0.029$ AP at IoU $0.50$ and decays monotonically, with the two methods crossing at IoU $0.70$; beyond that DetecTree2 leads, by as much as $0.035$ at IoU $0.80$. This is a resolution limit rather than a masker deficiency: with oracle boxes our mean per-crown mask IoU is $0.789$ (Table~\ref{tab:components}), and across matched true positives $39.2\%$ exceed IoU $0.75$ while only $20.6\%$ exceed $0.80$. The $8$~px feature grid bounds attainable IoU, so the method competes on foreground discrimination and spatial priors rather than on pixel-exact delineation.
...
Table~\ref{tab:strata} shows higher canopy coverage does not cause a decline in AP50. Mask AP50 is lowest on the most open tiles ($0.5519$ in the $[0,\,0.10)$ bin) and higher wherever canopy is more closed, peaking at $0.6824$ in the $[0.25,\,0.50)$ bin. The mask-minus-box gap, i.e. the masker's own contribution, stays between $+0.024$ and $+0.033$ across the four bins and in fact rises slightly as canopy becomes more dense, if anything.
...
\begin{table}[H]
\caption{Mean per-crown mask IoU within strata of matched \emph{box} IoU, OAM-TCD $439$-tile holdout. Computed on the $10{,}020$ crowns that every listed row matched, so the comparison is paired; the subset is defined by the row set, and adding or removing a row moves every number. \label{tab:strata-maskiou}}
\centering\footnotesize
\setlength{\tabcolsep}{4pt}
...
\bottomrule
\end{tabularx}
\noindent{\footnotesize{Canopy-landing predictions are dropped before matching. Stratifying on the \emph{detection} box rather than the tight box of the predicted mask is deliberate: a mask-derived box is partly determined by the mask, so an under-segmenting masker would be credited with a worse box. Per-stratum counts differ by row (each row's boxes distribute differently over the strata) and range from $303$ to $3{,}999$; the strata are populated for every row. LACE is seed $0$; over three detector seeds its mean mask IoU is $0.743 \pm 0.001$. LACE leads the two lowest strata and trails in the two highest --- where a box is nearly the ground-truth box, pixel-level supervision converts that into a better mask than an $8$~px lattice can express.}}
\end{table}
...
\paragraph{The canopy rule} COCO's crowd rule scores intersection over \emph{detection} area, so at a threshold of $0.50$ it coincides exactly with the ``more than half the prediction lies in canopy'' rule used elsewhere in this literature, and is strictly stricter above it. Both were computed and compared rather than assumed equivalent. Canopy-ignore is genuinely free: an ignored detection is removed from the true-positive and false-positive counts alike, so predicting into unlabelled canopy costs nothing. This matters most for the SelvaBox row, whose model never saw canopy during training and fires there freely; canopy-neutral scoring is what makes that row comparable at all.
...
\paragraph{Where the edge-band cull would have hurt} CanopyRS drops any polygon not wholly inside a subtile shrunk by $5\%$. Interior seams tolerate this, but the shrink also applies to the outer frame of the $2048^2$ tile, where no neighbouring subtile can recover the loss: $4{,}108$ of $25{,}705$ ground-truth crowns ($16.0\%$) touch that frame, capping recall at $0.84$ before the model predicts anything. It is also absent from the path that produced the authors' published numbers, whose tile-level AP is computed on raw model output.

\subsection{Asymmetries we state rather than hide}
...
\paragraph{The scoring raster favours coarse maskers} Reducing every mask to $512^2$ discards boundary error that a pixel-level model would otherwise be credited for avoiding. The ground-truth-box experiment of Table~\ref{tab:gtbox} measures the size of this: moving from $512$ to the native $2048$ raster costs our module $0.016$ mean IoU against SAM~3's $0.009$, and our advantage at IoU~$\ge 0.9$ falls from $2.4\times$ to $1.4\times$. The $512$ raster is thus the one protocol choice that plausibly flatters an $8$~px-lattice masker, which is why that experiment is reported at both rasters and the native one taken as its headline.

\paragraph{Seeds} LACE is reported over three detector seeds and, where a claim depends on it, its worst seed. Restor and SelvaBox are single released checkpoints with no seed variation available. DetecTree2 and Box2Mask are different: we train those ourselves, so seeds were available and simply not bought.

\paragraph{Domain gap in the fine-tuned SAM 3 arm} The \texttt{sam3-multi-selvabox-selvamask-FT} checkpoint was fitted on tropical UAV imagery at $1.3$--$3.5$~cm/px and is applied here at $10$~cm/px. This is context for interpreting that row, not a correction applied to it.
