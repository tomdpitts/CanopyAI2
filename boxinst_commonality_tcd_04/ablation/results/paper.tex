...
heads. The ranking rule contributes no fitted quantity
(Section~\ref{sec:scorer}).\label{tab:config}}
...
equivalent deletion of the canopy class. Every row was produced and
scored by us under one frozen protocol, with the single exception marked in note~c.
\emph{Imgs} is the number of OAM-TCD training images seen.}
\label{tab:oamtcd439}
...
\begin{tabularx}{\textwidth}{@{}M S r cc c@{}}
\toprule
...
DetecTree2 (fine-tuned) & masks & 900 & 0.534 & 0.224 & 0.531 \\
\addlinespace[2pt]
SelvaBox $\rightarrow$ SAM 3 (both FT)\tnote{b} & box + ext + SAM & $\sim$3335
  & 0.569 & 0.203 & \textbf{0.730} \\
SelvaBox $\rightarrow$ SAM 3 (frozen SAM)\tnote{c} & box + SAM & $\sim$3335
  & & 0.122 & \\
\addlinespace[2pt]
\textbf{LACE (ours)} (3-seed) & box & 900
  & \textbf{0.663} & \textbf{0.279} & 0.640 \\
 & & & {\footnotesize$\pm$0.001} & {\footnotesize$\pm$0.004}
     & {\footnotesize$\pm$0.024} \\
\bottomrule
\end{tabularx}
\begin{tablenotes}[flushleft]\footnotesize
\item[a] Released \texttt{restor/tcd-mask-rcnn-r50} checkpoint 
(\texttt{RPN.PRE/POST\_NMS\_TOPK\_TEST}~$=1000$); the shipped configuration halves this to 512
against tiles holding up to 450 crowns, which starves the second stage and yields 0.571 / 0.258
canopy-neutral. Restor predicts two classes; the figures above are for its tree class, and pooling
both classes as instances gives 0.614 / 0.274.
\item[b] OAM-TCD benchmark configuration ($1024^2$ subtiles at
0.5 overlap, native 0.1~m/px), the same grid as the DetecTree2 row. At their own detection budget
(maxDets 400 per subtile, 1600 per $2048^2$-equivalent) it reaches 0.577 / 0.205 canopy-neutral and
0.748 box AP$_{50}$.
\item[c] Not reproduced, quoted under SelvaMask protocol
(Table~\ref{tab:scorer}).
\end{tablenotes}
\end{threeparttable}
\end{table}


Tables~\ref{tab:oamtcd439} and~\ref{tab:neon-detection} summarize our model's performance across mAP (COCO mean Average Precision) for OAM-TCD and area/pixel-based semantic segmentation metrics for NEONTreeEvaluation, following the conventions established by the dataset authors' original publications. We report evaluations alongside baseline model results for comparison. For simplicity, consistency, and replicability, we use seed values starting at 0 and incremented by 1 (i.e. 0,1,2 for 3 seed). Every baseline in Table~\ref{tab:oamtcd439} was re-run and re-scored by us under a single protocol (with the exception noted). Each baseline was run in its own native prediction geometry, so only the measurement is held fixed, and we comment in the table notes on any configuration choice that materially affects a row. 
...
To test model efficacy on Savanna and Dryland biomes, we take a 'sparse-canopy' slice of the OAM-TCD data: $236$ tiles carrying $14{,}937$ crowns, none of which appears among the $900$ training images. No model is retrained for it -- the saved per-seed predictions are simply re-scored. 

\begin{table}[H]
\caption{Instance segmentation on the sparse-canopy OAM-TCD holdout ($236$ tiles, $14{,}937$
crowns, no tile shared with the $900$ training images). Canopy is scored as \texttt{iscrowd}
throughout, as in Table~\ref{tab:oamtcd439}. $\Delta$ is LACE minus DetecTree2. LACE bands are
$\mathrm{ddof}=1$ over detector seeds $0$--$2$; DetecTree2 is deterministic at a single
seed.\label{tab:sparse236}}
...
\midrule
DetecTree2 (fine-tuned, full masks) & $0.5531$ & $0.1548$ & $0.2371$ & $0.5340$ \\
...
\midrule
$\Delta$ & $+0.138$ & $+0.078$ & $+0.078$ & $+0.110$ \\
\bottomrule
\end{tabularx}
\noindent{\footnotesize{Scored with the same frozen COCOeval protocol as
Table~\ref{tab:oamtcd439} (pycocotools, masks at $512^2$, maxDets $600$, canopy as
\texttt{iscrowd}), against the sparse ground truth rather than the $439$. LACE leads on every
metric and on every seed. Both methods score higher here than on the full $439$ (LACE $0.691$
against $0.663$; DetecTree2 $0.553$ against $0.534$), so the open-canopy regime is easier for both
and the margin is not an artefact of one model preferring it; the margin is nonetheless larger
here ($+0.138$) than on the $439$ ($+0.128$). Sixteen of the $236$ tiles are drawn from the $439$
itself; they were never trained on, and the three inference scalars were selected on the $108$
validation images rather than on the $439$ (Section~\ref{sec:inference}).}}
\end{table}
...
\midrule
\multirow{3}{*}{Sparse 236} & DetecTree2 (FT) & $0.553$ & $0.155$ & $0.237$ \\
